...
| **T5-bis** | 🟠 | M | ⬜ | **Émission : sortir l'envoi SMTP de la réponse.** ≈ 3,4 s mesurés entre `genere_le` et `envoye_le` sur SAVV-2026-000023, pendant lesquels l'internaute attend une réponse qui ne dépend plus de lui. Même motif que l'instrumentation analytique déjà différée par `after()`. | `lib/db/certificatEmission.ts:307` (`publierEnvoiCertificat` dans le fil de la réponse) |
