...
| **T13** | 🟠 | S | ✅ | **Double requête au téléchargement d'un document, la seconde en 401** (journal du 22/09, iPhone via tunnel : `?doc=visuel&telecharger=1` → **200**, puis la MÊME URL → **401**). **`SameSite` est HORS DE CAUSE** : le cookie internaute est **`Lax`, pas `strict`** (`lib/internaute/authSession.ts:31`, qui le commente explicitement) — `Lax` envoie le cookie sur toute requête same-origin, téléchargement compris. **CE QUE RISQUE L'INTERNAUTE** (mesuré) : la réponse 401 est un JSON de **29 octets** (`{"erreur":"non authentifié"}`, `application/json`) — si c'est elle qui alimente l'enregistrement, le fichier obtenu est **illisible, sous un nom `.png`**, sans message d'erreur. **Correctifs posés** (`2db2cb7`) : **A** — retrait de l'attribut `download` du bouton, redondant avec le `Content-Disposition: attachment` du serveur et responsable du chemin « téléchargement piloté par le navigateur » qui rejoue la requête sur iOS ; **B** *(non retenu)* ; **C** — toutes les réponses d'erreur de la route portent `Cache-Control: private, no-store` comme ses 200, pour qu'aucun cache ne resserve une erreur à la place d'un document. **A et C n'ont PAS suffi** : nouveau constat du 22/09 (certificat 469) — même sans `download`, Safari iOS AFFICHE le PDF au lieu de l'enregistrer et rejoue la requête sans session. **RÉSOLU par la feuille de partage** (`3d6f159`, cf. **T14**) : le document est récupéré UNE fois au montage, et le clic ne déclenche plus AUCUNE requête réseau — la seconde requête ne peut plus exister. | `espace/ApercuDocument.tsx` (lien sans `download`) ; `api/internaute/espace/certificats/[id]/telecharger/route.ts` (`erreur()`, `sansCache()`) ; preuves : `espace/ApercuDocument.test.tsx`, `…/telecharger/route.test.ts` |
| **T13-bis** | 🟠 | M | ⏸️ **ABANDONNÉ** | **Jeton de téléchargement à usage unique — plan B, devenu SANS OBJET** (22/09). Il visait à faire porter l'accès par un jeton plutôt que par le cookie, parce qu'une SECONDE requête partait sans session. La feuille de partage (**T14**) supprime cette seconde requête : le fichier est déjà en main au moment du clic. Plus rien à porter, donc plus de jeton à émettre, expirer et rejouer — surface évitée. ⚠️ À ne PAS rouvrir « au cas où » : le rouvrir n'aurait de sens que si un chemin réintroduisait une requête réseau au clic. | sans objet — cf. `espace/ApercuDocument.tsx` |
| **T14** | 🟠 | S | ✅ | **Le bouton « Télécharger ce document » ouvre la FEUILLE DE PARTAGE native quand l'appareil la propose** (`3d6f159`) — décision du porteur du 22/09 : iPhone → Enregistrer dans Fichiers, Enregistrer l'image dans Photos, Messages, WhatsApp, mail… ; ordinateur ou partage de fichiers indisponible → téléchargement, comme avant. **Le document est récupéré UNE SEULE FOIS au montage** (`fetch` same-origin, donc avec le cookie) et devient un `File` (nom lu dans `Content-Disposition`, type réel) : ce même fichier sert à l'aperçu ET au bouton → **aucune requête réseau au clic**, donc plus de seconde requête possible (cf. T13). `navigator.share` est appelé **dans l'élan direct du geste, sans aucun `await` avant** — Safari refuse l'appel autrement, et c'est la raison d'être de la récupération au montage. Annulation (AbortError) = silencieux ; autre refus = repli sur l'enregistrement depuis l'URL d'objet. États : « Préparation… » (bouton inactif) → « Télécharger ce document » ; 401 → « session expirée » + lien de reconnexion ; autre erreur → « document non préparé ». ⚠️ **Aucune branche serveur/client au rendu** (ni `typeof window`, ni `navigator`) : une divergence d'attribut avait provoqué une erreur d'hydratation en dev. Garde d'accès et route serveur inchangées. | `espace/ApercuDocument.tsx` ; preuves `espace/ApercuDocument.test.tsx` (partage, annulation, repli sans requête, hydratation) |
| **T15** | 🟠 | M | ⏸️ **ÉCARTÉ — décision produit (Arno, 22/09)** | **Sortir la fabrication des documents (carte, PDF) de la réponse d'émission — NON RETENU.** Le correctif était techniquement prêt à proposer : répondre dès que le certificat est FRAPPÉ en base (l'acte qui fait foi), puis fabriquer carte → PDF → mail derrière, ce qui ramènerait la réponse de ~3 s à ~0,3-0,5 s. **Arno a tranché l'inverse** : l'internaute attend ~3 s **mais trouve ses documents PRÊTS immédiatement**, sans jamais rencontrer d'état « en préparation ». L'attente est jugée préférable à une promesse différée sur un document qui fait foi. ⚠️ **Ne pas reproposer ce correctif comme « à faire »** : ce n'est pas une dette, c'est un arbitrage. Il ne se rouvrirait que si la fabrication redevenait longue au point de gêner (cf. T16). Conséquence assumée : **~3 s incompressibles** dans la réponse, dont la carte d'orientation est la part dominante, et qui dépendent d'un service externe (tuiles IGN). | `lib/db/certificatEmission.ts:317-320` (carte puis PDF, avant la réponse — INCHANGÉ, volontairement) |
| **T16** | 🟡 | — | 📌 **CONSTAT, cause non établie** | **Une émission à 14,5 s le 22/09 à 16:14 (`SAVV-2026-000029`) — AUCUN défaut de code trouvé.** Vérifications faites : le serveur tournait bien **sur le code à jour** (preuve matérielle : le PDF déposé pèse **370 Kio**, donc `f6dbb96` était actif ; 000027 et antérieurs pèsent ~996 Kio) ; **`c5c01ed` fonctionnait** (réponse 14,5 s ≈ `emis_le`→`genere_le` 14,06 s : le mail, 8,62 s, était bien HORS de la réponse — sans lui la réponse aurait duré ~22,7 s). Le PDF prend ~0,18 s → **les ~13,9 s restantes sont la carte**, alors que la même carte mesurée juste après prend **2,5-3,1 s** et que 000028, même code, l'a faite en 4,81 s. **Fait décisif : les DEUX étapes réseau ont ralenti ENSEMBLE** — carte ×4,5 et SMTP ×2,8, au même instant. **Hypothèse principale : dégradation passagère du lien réseau du Mac** (seule à expliquer que deux services externes indépendants — tuiles IGN et serveur SMTP — ralentissent dans le même rapport). Écartées : ralentissement IGN seul (n'explique pas le SMTP), contention machine (aucun `npm test` en cours à 16:14:52), recompilation Next à froid (n'explique pas le SMTP, exécuté après la réponse). **Rien à corriger** ; à rouvrir seulement si le cas se répète sur un réseau sain. | horodatages `certificat_acheminement` ; poids des PDF déposés en stockage |
...
| 2026-09-22 | Décision d'émission + constat des 14,5 s | ⏸️ **T15 — décision d'Arno** : on NE sort PAS la fabrication des documents (carte, PDF) de la réponse d'émission. L'internaute attend ~3 s **mais trouve ses documents prêts immédiatement**, sans état « en préparation ». Le correctif est classé **écarté par décision produit**, pas « à faire » — ne pas le reproposer comme une dette. 📌 **T16 — constat** : l'émission de `SAVV-2026-000029` à 16:14 a pris 14,5 s, **sans aucun défaut de code** (code à jour prouvé par le poids du PDF déposé, 370 Kio ; `c5c01ed` actif). Les deux étapes réseau ont ralenti ensemble (carte ×4,5, SMTP ×2,8) → **hypothèse principale : dégradation passagère du réseau du Mac**. Cause non établie, rien à corriger. |
| 2026-09-22 | T14 — feuille de partage pour le téléchargement d'un document | Les correctifs A et C (`2db2cb7`) **n'ont pas suffi** : nouveau constat sur iPhone (certificat 469) — même sans `download`, Safari AFFICHE le PDF au lieu de l'enregistrer et rejoue la requête sans session. ✅ **T14** (`3d6f159`) : le document est récupéré **une seule fois au montage** et le bouton ouvre la **feuille de partage** quand l'appareil la propose, sinon enregistre le fichier déjà en main — **plus aucune requête réseau au clic**, la seconde requête ne peut donc plus exister. `navigator.share` appelé dans l'élan direct du geste (exigence Safari). Aucune branche serveur/client au rendu (une erreur d'hydratation avait été vue en dev). ⏸️ **T13-bis (jeton de téléchargement) ABANDONNÉ** : devenu sans objet, il n'y a plus de requête à authentifier au clic. `npm test` : 597 fichiers / 7916 tests verts. |
