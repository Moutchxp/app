...
aujourd'hui placeholder), les **39 colonnes du singleton `config_scoring` (id=1)**, **en LECTURE SEULE
...
- **Responsive §15** : le tableau des 39 variables DOIT être lisible et utilisable sur iPhone portrait.
...
- **D2 — Regroupement PAR FAMILLE** à l'affichage, reflétant les familles réelles des 39 colonnes
...
- **EX-1** [Ubiquitaire] Le système DOIT exposer une route **`GET`** à `/api/admin/config` renvoyant les
  **39 colonnes** de `config_scoring` pour `id=1`.
...
- **EX-8** [Ubiquitaire] La page Pilotage DOIT afficher les 39 variables **regroupées par famille** (D2),
...
| **T2** | Affichage groupé + formaté | Les 39 colonnes rendues, groupées par famille ; chaque ligne = libellé + unité + **nom technique** + statut + valeur + défaut ; **aucun** `<input>/<button save>` (grep DOM) ; tokens `svv-*` présents. |
