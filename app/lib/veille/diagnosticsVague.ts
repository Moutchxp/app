 ...
 *
 * Une mairie peut envoyer les documents en plusieurs mails. On recense les dossiers PARTIELS (dossier partiel actif) dont la GED a
 ...
 * LEVER le marqueur partiel (CASC-1). AUCUN envoi n'est déclenché ici (le diagnostic ne réclame rien : c'est PART-E).
 *
...
/** Un dossier PARTIEL dont la GED a changé, avec la date d'envoi du dernier mail de sa demande (null = aucun). */
...
export interface DepsDiagnosticsVague {
  ...
  candidats(): Promise<CandidatVague[]>;           // dossiers partiels à GED changée + dernier mail de la demande
...
/**
 * Pour chaque dossier partiel à GED changée : si la vague est close (selon le mode), lance UN diagnostic ; sinon le REPORTE.
...
/**
 * Candidats réels : dossiers ACTIFS de demandes en DOSSIER PARTIEL actif (partiel_le renseigné, non levé) dont le nombre de
 * documents GED (hors fiche de synthèse) DIFFÈRE du nombre mémorisé au dernier diagnostic — donc de nouvelles pièces ont été
 * versées depuis (MÊME critère de péremption que `lireCompletude`). `dernierMailLe` = date d'ENVOI du dernier mail non-rebond de
 * la demande. RÉSILIENT : colonnes/tables absentes (177/174) → aucun candidat (jamais d'exception propagée).
 */
export async function candidatsVagueReels(): Promise<CandidatVague[]> {
  try {
    const { rows } = await query<{ dossier_id: number; demande_id: number; dernier_mail_le: Date | null; nb_ged: number; nb_memorise: number | null }>(
      `SELECT dd.dossier_id::int AS dossier_id, d.id::int AS demande_id,
        ...
        WHERE d.partiel_le IS NOT NULL AND d.partiel_leve_le IS NULL`,
  ...
  } catch { return []; } // 177/174 absentes → aucun candidat (résilient)
}
