 ...
 * formulaire) dont il cite le permis : num_dau (normalisé, lettres conservées) OU référence mairie (P1). Sépare deux files DISTINCTES :
...
export function apparierPropositions(messages: MessageCandidat[], cibles: CibleDepot[]): { propositions: PropositionDepot[]; idsCitants: Set<number> } {
  ...
  for (const m of messages) {
    const foinNum = normaliserNumeroDossier(`${m.objet ?? ''}\n${m.corpsTexte ?? ''}\n${m.nomsPieces.join('\n')}`);
