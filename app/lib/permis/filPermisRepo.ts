...
import { MOTIF_COMPLEMENT_PREFIXE, MOTIF_DECLARATION_PREFIXE } from './demanderPiecesRepo';
...
export function depsReellesFil(): DepsFil {
  return {
    ...
    entreesDesDemandes: async (demandeIds) => {
      ...
      // 1) REÇUS (hors rebond) — avec leur corps.
      const { rows: recus } = await query<{ le: string; interlocuteur: string | null; objet: string | null; corps: string | null }>(
        `SELECT to_char(recu_le AT TIME ZONE 'UTC', ${ISO_UTC}) AS le, coalesce(nullif(de_nom, ''), de_adresse) AS interlocuteur, objet, corps_texte AS corps
           FROM demande_reponse WHERE demande_id = ANY($1) AND nature <> 'rebond'`, [demandeIds]);
      for (const r of recus) entrees.push({ le: r.le, sens: 'recu', interlocuteur: r.interlocuteur, objet: r.objet, corps: r.corps, corpsConnu: true });
