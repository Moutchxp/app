 ...
 *
 * ORDRE (rang décroissant) :  saisie > cerfa > enonce > plan > ia > motifs
 ...
 *   - motifs  : la COTE ISOLÉE glanée dans le texte — le plus faible, dernier recours.
 *
...
/** Les méthodes automatiques journalisées (liste fermée du CHECK migrations 109/133). 'saisie' n'y figure pas (jamais journalisée). */
export type MethodeExtraction = 'cerfa' | 'enonce' | 'plan' | 'ia' | 'motifs';

/** Rang décroissant : indice 0 = plus fort. `saisie` inclus pour l'ordre complet, mais l'invariant 103 la traite via l'origine. */
export const PRECEDENCE_METHODES = ['saisie', 'cerfa', 'enonce', 'plan', 'ia', 'motifs'] as const;
