...
export interface ConfigVeille {
  ...
  familleAttendueCerfa: boolean;    // PART-2 : attendre un formulaire Cerfa dans les pièces d'un permis ? (défaut true)
  ...
  cascadePartielAutoActive: boolean;  // AUTO-PARTIEL : les étapes CASC-3 partent SEULES aux dates de la cascade (défaut TRUE) — interrupteur d'arrêt d'urgence
  relanceMultiAdresseActive: boolean;      // LOT 20/27 : envoyer les N dernières relances à TOUTES les adresses connues de la commune (LOT 27 : défaut TRUE = norme ; drapeau = arrêt d'urgence)
...
// PART-2 — les 4 familles ATTENDUES (interrupteurs). Lecture ISOLÉE et résiliente à l'ordre d'application de la 174 : colonnes
//   absentes → les 4 à true (défaut), sans dégrader le reste.
async function lireFamillesAttendues(): Promise<Pick<ConfigVeille, 'familleAttendueCerfa' | 'familleAttendueMasse' | 'familleAttendueCoupe' | 'familleAttendueEtage'>> {
  const def = { familleAttendueCerfa: true, familleAttendueMasse: true, familleAttendueCoupe: true, familleAttendueEtage: true };
  try {
    const { rows } = await query<{ c: boolean | null; m: boolean | null; p: boolean | null; e: boolean | null }>(
      `SELECT famille_attendue_cerfa AS c, famille_attendue_masse AS m, famille_attendue_coupe AS p, famille_attendue_etage AS e FROM config_veille WHERE id = 1`);
    const r = rows[0];
    if (!r) return def;
    return {
      familleAttendueCerfa: r.c ?? true, familleAttendueMasse: r.m ?? true,
      familleAttendueCoupe: r.p ?? true, familleAttendueEtage: r.e ?? true,
    };
...
// CASC-2 — lecture ISOLÉE du délai avant saisine CADA sur DOSSIER PARTIEL (résiliente à l'ordre d'application de la 178, livrée NON
//   APPLIQUÉE) : tant que les colonnes n'existent pas, cette lecture échoue SEULE et retombe sur (1 mois, 4 jours), SANS dégrader le reste.
async function lireCadaPartielDelai(): Promise<Pick<ConfigVeille, 'cadaPartielDelaiMois' | 'cadaPartielDelaiJours'>> {
  const def = { cadaPartielDelaiMois: 1, cadaPartielDelaiJours: 4 };
  try {
    const { rows } = await query<{ cada_partiel_delai_mois: number; cada_partiel_delai_jours: number }>(
      `SELECT cada_partiel_delai_mois, cada_partiel_delai_jours FROM config_veille WHERE id = 1`);
    const r = rows[0];
    if (!r) return def;
    return { cadaPartielDelaiMois: r.cada_partiel_delai_mois, cadaPartielDelaiJours: r.cada_partiel_delai_jours };
...
// AUTO-PARTIEL — interrupteur de l'envoi automatique de la cascade partielle. Lecture ISOLÉE (résiliente à la 184, livrée NON APPLIQUÉE) :
//   colonne absente → défaut TRUE (intention d'Arno), SANS dégrader le reste de la config.
async function lireCascadePartielAuto(): Promise<Pick<ConfigVeille, 'cascadePartielAutoActive'>> {
  try {
    const { rows } = await query<{ cascade_partiel_auto_active: boolean }>(`SELECT cascade_partiel_auto_active FROM config_veille WHERE id = 1`);
    const r = rows[0];
    if (!r) return { cascadePartielAutoActive: true };
    return { cascadePartielAutoActive: r.cascade_partiel_auto_active === true };
  } catch { return { cascadePartielAutoActive: true }; } // 184 pas encore appliquée → défaut ACTIF
}

// LOT 20 — lecture ISOLÉE du multi-adresse (résiliente à l'ordre d'application de la 182, livrée NON APPLIQUÉE) : colonnes absentes → OFF + 2.
async function lireRelanceMultiAdresse(): Promise<Pick<ConfigVeille, 'relanceMultiAdresseActive' | 'relanceMultiAdresseNbDernieres'>> {
  const def = { relanceMultiAdresseActive: true, relanceMultiAdresseNbDernieres: 2 }; // LOT 27 : défaut TRUE (norme) ; colonnes absentes → multi-adresse actif
  try {
    const { rows } = await query<{ relance_multi_adresse_active: boolean; relance_multi_adresse_nb_dernieres: number }>(
      `SELECT relance_multi_adresse_active, relance_multi_adresse_nb_dernieres FROM config_veille WHERE id = 1`);
    const r = rows[0];
    if (!r) return def;
    return { relanceMultiAdresseActive: r.relance_multi_adresse_active === true, relanceMultiAdresseNbDernieres: r.relance_multi_adresse_nb_dernieres };
  } catch { return def; } // 182 pas encore appliquée → OFF (opt-in)
}
...
export async function chargerConfigVeille(): Promise<ConfigVeille> {
  try {
    ...
    return {
      ...
      ...(await lireSurveillanceConfig()),              // SURV-1 : tolérance + fenêtre de surveillance, lecture isolée (résiliente à la 171)
    };
  } catch {
