/**
 * Table de correspondance UI pour les 39 colonnes du singleton `config_scoring`
...
// Les 8 colonnes du barème par année (a1900_*/a1935_*/borne_annee_1900/1935) ont été SUPPRIMÉES
// (migration 008) : le barème par année est désormais piloté par les cartes d'année dynamiques
// (table `config_famille_annee`, CRUD).

/**
 * Les 39 colonnes de `config_scoring`, ordonnées par famille.
 * Récapitulatif : 31 VIVE · 5 VESTIGIALE · 1 DE GARDE · 1 MIROIR · 1 technique = 39.
 * (Les 8 colonnes du barème par année — a1900_cone/flanc/distmax, a1935_cone/flanc/distmax,
 *  borne_annee_1900 et borne_annee_1935 — ont été SUPPRIMÉES (migration 008) au profit des cartes
 *  d'année dynamiques (`config_famille_annee`). `config_scoring` passe de 47 à 39 colonnes.)
 */
export const META: readonly ColonneMeta[] = [
  ...
  { colonne: 'analysis_range_m', libelle: 'Portée d’analyse — borne maximale du plafond de distance perçue (verrouillée)', unite: 'mètres', famille: F_PORTEE, statut: 'MIROIR', defaut: 200, type: 'entier', editable: false, min: 1, max: 2000, pas: 1, infobulle: 'Verrou de sécurité, en lecture seule. Son rôle réel : fixer la valeur maximale autorisée pour le plafond de distance perçue par faisceau — si ce plafond dépasse cette borne, toute la configuration est rejetée et le moteur repasse aux valeurs par défaut. Elle N’ÉTEND PAS le rayon de scan des obstacles (fixé indépendamment) : l’augmenter n’ouvrirait la vue sur rien de plus loin, mais autoriserait un plafond de distance perçue plus haut — ce qui déplacerait les scores. Elle reste donc verrouillée à sa valeur.' },
