...
const RIEN = { aEnregistrer: [], altitudeAValider: [] };
...
describe('statutBatimentsProjection — fraîcheur ajoutée sans écraser l’état serveur', () => {
  ...
  it('base VERTE + 1 bâtiment à enregistrer → ROUGE nommant le bâtiment', () => {
    const r = statutBatimentsProjection(VERT, { aEnregistrer: ['B1'], altitudeAValider: [] });
  ...
  it('base VERTE + 1 altitude à valider → ROUGE nommant le bâtiment', () => {
    const r = statutBatimentsProjection(VERT, { aEnregistrer: [], altitudeAValider: ['B2'] });
    expect(r.ton).toBe('rouge');
    expect(r.texte).toContain('1 altitude à valider (B2)');
  });

  it('base VERTE + les DEUX exigences manquantes → ROUGE cumulant les deux détails', () => {
    const r = statutBatimentsProjection(VERT, { aEnregistrer: ['B1'], altitudeAValider: ['B3'] });
    expect(r.ton).toBe('rouge');
    expect(r.texte).toContain('à enregistrer (B1)');
    expect(r.texte).toContain('à valider (B3)');
  });

  it('base déjà ROUGE (emprise) + 1 à enregistrer → conserve le manquement serveur ET ajoute le nouveau', () => {
    const r = statutBatimentsProjection(ROUGE_EMPRISE, { aEnregistrer: ['B1'], altitudeAValider: [] });
    expect(r.ton).toBe('rouge');
  ...
  it('plus de 2 bâtiments → abrège « 2 premiers et N autre(s) »', () => {
    const r = statutBatimentsProjection(VERT, { aEnregistrer: ['B1', 'B2', 'B3', 'B4'], altitudeAValider: [] });
    expect(r.texte).toContain('4 bâtiments à enregistrer (B1, B2 et 2 autres)');
  });

  it('les manquements PLURIELS s’accordent', () => {
    const r = statutBatimentsProjection(VERT, { aEnregistrer: ['B1', 'B2'], altitudeAValider: ['B3', 'B4'] });
    expect(r.texte).toContain('2 bâtiments à enregistrer (B1, B2)');
    expect(r.texte).toContain('2 altitudes à valider (B3, B4)');
  });
});
