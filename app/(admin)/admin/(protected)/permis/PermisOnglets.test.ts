...
describe('S13 — barre d’onglets Permis (deux groupes nommés)', () => {
  ...
  it('les onglets sont dans le BON groupe et dans le BON ordre', () => {
    ...
    expect(GROUPES_ONGLETS[0].onglets.map((o) => o.cle)).toEqual(['dossiers', 'automatisation', 'rattachement', 'sous_surveillance']);
    expect(GROUPES_ONGLETS[1].onglets.map((o) => o.cle)).toEqual(['a_demander', 'en_cours', 'reponses', 'projection', 'archives', 'saisines', 'collaborateurs', 'reglages']);
    // « Sous surveillance » juste APRÈS « Rattachement » (radar séparé du travail). Ordre RÉEL dans le markup :
    const h = rendu('dossiers');
    let pos = -1;
    for (const lib of ['Dossiers', 'Automatisation', 'Rattachement', 'Sous surveillance', 'À demander', 'En cours', 'Réponses', 'Analyse et projection', 'Archives', 'Saisines CADA', 'Collaborateurs', 'Réglages']) {
