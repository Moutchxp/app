...
describe('S13 — barre d’onglets Permis (deux groupes nommés)', () => {
  ...
  it('les onglets sont dans le BON groupe et dans le BON ordre', () => {
    ...
    expect(GROUPES_ONGLETS[0].onglets.map((o) => o.cle)).toEqual(['dossiers', 'automatisation', 'rattachement', 'trace_emprise']);
    ...
    // ordre RÉEL dans le markup : Dossiers, Automatisation, Rattachement, Tracé emprise, puis À demander, En cours, Réponses, Archives, Saisines CADA, Collaborateurs, Réglages
    const h = rendu('dossiers');
    let pos = -1;
    for (const lib of ['Dossiers', 'Automatisation', 'Rattachement', 'Tracé emprise', 'À demander', 'En cours', 'Réponses', 'Archives', 'Saisines CADA', 'Collaborateurs', 'Réglages']) {
