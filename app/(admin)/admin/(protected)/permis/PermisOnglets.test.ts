...
describe('S13 — barre d’onglets Permis (deux groupes nommés)', () => {
  ...
  it('les six onglets sont dans le BON groupe et dans le BON ordre', () => {
    ...
    expect(GROUPES_ONGLETS[1].onglets.map((o) => o.cle)).toEqual(['demandes', 'reponses', 'collaborateurs', 'reglages']);
    // ordre RÉEL dans le markup : Dossiers, Automatisation, puis Demandes, Réponses, Collaborateurs, Réglages
    const h = rendu('dossiers');
    let pos = -1;
    for (const lib of ['Dossiers', 'Automatisation', 'Demandes', 'Réponses', 'Collaborateurs', 'Réglages']) {
