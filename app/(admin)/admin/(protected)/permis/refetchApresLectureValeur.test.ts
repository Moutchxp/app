...
describe('BlocTraceEmprise — repasse le signal de sa liseuse embarquée au parent (canvas/tracé intouché)', () => {
  it('accepte onValeurLue et le branche sur onValeurEcrite de la liseuse embarquée', () => {
    ...
    expect(TRACE).toContain('<LiseusePieces dossierId={dossierId} onValeurEcrite={onValeurLue} donneesPrechargees={donneesLiseuse} titreEnEntete imageAgrandie={imageAgrandie} onToggleImageAgrandie={() => { setPleinEcran(false); setImageAgrandie((v) => !v); }} />'); // P3 donnée partagée + agrandi piloté par le parent, mutuellement exclusif du schéma (deux boutons)
  });
});
