 ...
 *  · la FERMETURE purge une session intouchée — `fermerPleinEcran` appelle `purgerAjustementIntouche` ;
...
describe('BlocTraceEmprise — armement des poignées à l’ouverture, purge à la fermeture (plein écran)', () => {
  ...
  it('fermerPleinEcran PURGE une session intouchée (purgerAjustementIntouche)', () => {
    const bloc = corps('fermerPleinEcran');
    expect(bloc).toContain('setPleinEcran(false)');
    expect(bloc).toContain('purgerAjustementIntouche()');
  });
  ...
  it('la purge ne concerne qu’une session SINGLE intouchée (jamais le mode bloc ni un travail modifié)', () => {
    const bloc = corps('purgerAjustementIntouche');
    expect(bloc).toContain('!a.bloc');                 // le geste d'ENSEMBLE (explicite) n'est pas purgé
    expect(bloc).toContain('estAjustementModifie');    // une session MODIFIÉE (travail non enregistré) est conservée
  });
});
