...
export function BlocTraceEmprise({ dossierId, onVerdict, rafraichir = 0, avecLiseuse = true, onValeurLue, onEmprisesChange, onEntete, onDonneesLiseuse, demandeAcces = null, onDemandeConsommee }: {
  ...
  // POINT 1 — VALIDATION PAR BÂTIMENT recomposée CÔTÉ CLIENT depuis la SEULE vérité par emprise (`e.validee`), comme la rangée d'actions. On
  //   n'utilise PLUS le signal serveur `validationParCorps` (qui mêlait la validation legacy niveau permis / pointeur corps 206 à la validation
  //   par emprise 213) : bandeau, en-tête, onglet cartouche et rangée d'actions dérivent ainsi du MÊME état → jamais contradictoires.
  const validationParCorps = useMemo(() => validationParCorpsDepuisEmprises(emprises), [emprises]);
  ...
  // LETTRAGE — « ce cleabs est-il un repère en projet ? » (règle pure, source unique) pour la nomenclature rouge des LISTES textuelles (AdoptionGroupes).
  const enProjetParCleabs = useMemo(() => { const m = new Map<string, boolean>(); for (const p of polygonesReperes) if (p.cleabs) m.set(p.cleabs, estReperePolygoneEnProjet(p)); return m; }, [polygonesReperes]);
      ...
      {batSel && (
        ...
        <div role={imageAgrandie || planSeul ? 'dialog' : undefined} aria-modal={imageAgrandie || planSeul || undefined}
          ...
          <div key="schema" className="svv-card" style={{ display: 'flex', flexDirection: 'column', gap: '.5rem', minWidth: 0 }}>
            ...
            <AdoptionGroupes groupes={groupesAdoption} batiments={batiments} reperes={reperesParCleabs} affectation={affectation} scindes={scindes} occupe={occupe} enProjetDe={(c) => enProjetParCleabs.get(c) ?? false}
