...
const emprise = (over: Partial<EmpriseReconstruite> = {}): EmpriseReconstruite => ({
  ...
  surfaceM2: 100, pieceId: 55, page: 2, calage: null, residuM: 0.3, provenance: 'trace_manuel', ajustement: null, ajustementParNom: null, creeLe: null, numero: null, validee: false, valideeLe: null, valideePar: null, valideeParNom: null, ...over,
});
...
describe('PROJ-3h/3i — options, repères, sélection des polygones « en projet »', () => {
  ...
  it('④ LegendeSchemaProjection : catégories + picto « i » (explication)', () => {
    ...
    expect(html).toContain('#0f766e'); // ① teal (permis — inchangé)
    expect(html).toContain('#2563eb'); // ② BLEU (bâtiment voisin hors permis)
    expect(html).toContain('#7ab4e6'); // ③ BLEU CLAIR (parcelle voisine)
    ...
    expect(html).toContain('il ne reçoit PAS de repère et n’entre PAS dans l’affectation'); // explication du voisin hors permis
  ...
  it('RÈGLE ARNO — le bâtiment DU PERMIS (teal, repère A) vs le VOISIN hors permis (BLEU, SANS repère)', () => {
    ...
    const polygones = attribuerReperes([
      ...
      { cleabs: 'HORS', anneau: carre(16, 16, 12), etat: 'En service', appartientPermis: false },
    ]);
    ...
    // ① le bâtiment DU PERMIS : teal + contour ÉPAIS (1.9) → dominant (INCHANGÉ).
    const permisTag = html.match(/<path\b[^>]*fill="rgba\(15,118,110,\.34\)"[^>]*>/)?.[0] ?? '';
    expect(permisTag).toContain('stroke="#0f766e"');    // teal (couleur FIXE, jamais un token)
    expect(permisTag).toContain('stroke-width="1.9"');  // contour le plus épais → domine
    // ② le VOISIN hors permis : BLEU + contour plus FIN (1.3), et l'attribut d'appartenance à false.
    const voisinTag = html.match(/<path\b[^>]*data-appartient-permis="false"[^>]*>/)?.[0] ?? '';
    expect(voisinTag).toContain('stroke="#2563eb"');    // BLEU (bâtiment voisin hors permis)
    expect(voisinTag).toContain('fill="rgba(37,99,235,.18)"');
    expect(voisinTag).toContain('stroke-width="1.3"');  // plus fin que le permis (1.9) → nettement second
    expect(permisTag).not.toContain('#2563eb');         // teintes DISTINCTES : permis ≠ voisin
    // le PERMIS garde son repère A ; le VOISIN n'a AUCUN repère (repérage restreint au permis, repart de A).
    expect(html).toContain('data-repere="A"');
    expect(html).not.toContain('data-repere="B"');
  });
});
...
describe('PROJ-CTX — contexte (parcelles voisines + bâti) : 3e registre, interrupteur, aucune requête si éteint', () => {
  ...
  it('(a) contexte ALLUMÉ (défaut) → parcelles voisines (bleu clair) + bâti voisin (bleu) RENDUS, non cliquables', () => {
    ...
    expect(html).toContain('stroke="#7ab4e6"');            // parcelle voisine = BLEU CLAIR (FIXE, lisible dans les 2 thèmes)
    expect(html).toContain('stroke="#2563eb"');            // bâti voisin = BLEU (même bleu que les voisins hors permis)
...
describe('LOT 81 — légende à DEUX GROUPES (polygones BD TOPO réels + emprises projetées) du schéma « Projection des emprises »', () => {
  ...
  it('abregerCleabs : abrège une longue clé (title garde la valeur complète), laisse une courte intacte', () => {
    expect(abregerCleabs('BATIMENT0000002493678242')).toBe('BATIMENT0000…678242');
...
describe('PROJ-3s — retouche : liste (retoucher / multi-parties) + poignées sur le schéma', () => {
  ...
  it('SchemaParcelleTrace (point 3) : l’emprise stockée n’est PLUS surlignée par sélection (data-selectionnee absent) — le surligné est l’aperçu transformé', () => {
    ...
    // Le surlignage figeait la géométrie STOCKÉE et divergeait de la cible dès un déplacement (point 3) : il est supprimé.
    // Rendre le schéma ne produit plus AUCUN data-selectionnee (la cible sélectionnée = l’aperçu d’ajustement, dessiné à part).
...
describe('LOT 99 — statut PAR PAGE à TROIS AXES (statutPageAnalyse PURE, source unique) : exclusif, exhaustif, honnête', () => {
  it('rien du tout → NON IDENTIFIÉE (nature aucune, origine null) — jamais un état trompeur', () => {
    expect(statutPageAnalyse({})).toEqual({ nature: 'aucune', origine: null, etat: 'non_identifiee', derive: false, dateLisible: null });
  });
  it('IA grain PAGE MESURÉ (permis_page_lecture) : valeurs lues / rien de lisible / écartée RGPD / non analysable', () => {
    expect(statutPageAnalyse({ lecturePage: { envoyee: true, nbValeurs: 1, motif: null, creeLe: 'Z' } }, () => '05/09/2026'))
      .toEqual({ nature: 'ia', origine: 'manuelle', etat: 'valeurs_lues', derive: false, dateLisible: '05/09/2026' });
    expect(statutPageAnalyse({ lecturePage: { envoyee: true, nbValeurs: 0, motif: null, creeLe: 'Z' } }).etat).toBe('rien_lisible'); // JAMAIS « non analysée » (ne pas faire repayer)
    expect(statutPageAnalyse({ lecturePage: { envoyee: false, nbValeurs: 0, motif: 'signature d’une personne détectée', creeLe: 'Z' } }).etat).toBe('ecarte_rgpd');
    expect(statutPageAnalyse({ lecturePage: { envoyee: false, nbValeurs: 0, motif: 'page sans texte exploitable', creeLe: 'Z' } }).etat).toBe('non_analysable');
  });
  it('page écartée pendant le REPÉRAGE (grain page mesuré) → écartée / non analysable selon le motif', () => {
    expect(statutPageAnalyse({ ecarteeReperage: { motif: 'date de naissance détectée' }, reperage: { creeLe: 'Z' } }).etat).toBe('ecarte_rgpd');
    expect(statutPageAnalyse({ ecarteeReperage: { motif: 'page sans texte' }, reperage: { creeLe: 'Z' } }).etat).toBe('non_analysable');
  });
  it('repérage IA du FICHIER (dérivé) → identifiée non lue, nature IA, DÉRIVÉ (d’après le fichier)', () => {
  ...
  it('PRIORITÉ : le MESURÉ (IA page) prime le DÉRIVÉ (repérage/identification) ; « lu » prime « identifié »', () => {
    ...
    expect(statutPageAnalyse({ lecturePage: { envoyee: true, nbValeurs: 2, motif: null, creeLe: 'A' }, reperage: { creeLe: 'B' }, identifieeSansIa: true }).etat).toBe('valeurs_lues');
    // repérage IA (nature ia) prime l'identification sans IA quand aucune lecture de page.
    expect(statutPageAnalyse({ reperage: { creeLe: 'B' }, identifieeSansIa: true }).nature).toBe('ia');
  });
  it('EXCLUSIF + EXHAUSTIF : chaque combinaison retenue tombe dans exactement UN état connu', () => {
    const etatsConnus = new Set(['non_identifiee', 'identifiee_non_lue', 'valeurs_lues', 'rien_lisible', 'non_analysable', 'ecarte_rgpd']);
    const cas: import('./TraceEmpriseRendu').EntreesStatutPage[] = [
      {}, { identifieeSansIa: true }, { reperage: { creeLe: 'Z' } },
      { lecturePage: { envoyee: true, nbValeurs: 1, motif: null, creeLe: 'Z' } },
      { lecturePage: { envoyee: true, nbValeurs: 0, motif: null, creeLe: 'Z' } },
      { lecturePage: { envoyee: false, nbValeurs: 0, motif: 'page sans texte', creeLe: 'Z' } },
    ];
    for (const c of cas) { const s = statutPageAnalyse(c); expect(etatsConnus.has(s.etat)).toBe(true); }
  });
  it('libelleStatutPage — FRANÇAIS CLAIR, « page identifiée mais non analysée » remplace « page couverte (fichier) »', () => {
    expect(libelleStatutPage({ nature: 'ia', origine: 'manuelle', etat: 'identifiee_non_lue', derive: true, dateLisible: null })).toBe('page identifiée mais non analysée');
    expect(libelleStatutPage({ nature: 'ia', origine: 'manuelle', etat: 'valeurs_lues', derive: false, dateLisible: null })).toBe('valeurs lues et intégrées');
    expect(libelleStatutPage({ nature: 'ia', origine: 'manuelle', etat: 'rien_lisible', derive: false, dateLisible: null })).toContain('aucune valeur lisible');
  });
  it('titreStatutPage — NATURE + ORIGINE + ÉTAT EN TOUTES LETTRES + date + « d’après l’analyse du fichier » si dérivé', () => {
    expect(titreStatutPage({ nature: 'sans_ia', origine: 'indeterminee', etat: 'identifiee_non_lue', derive: true, dateLisible: '05/09/2026' }))
      .toBe('page identifiée mais non analysée — analyse sans IA (texte du document), origine indéterminée, le 05/09/2026 (d’après l’analyse du fichier)');
    expect(titreStatutPage({ nature: 'ia', origine: 'manuelle', etat: 'valeurs_lues', derive: false, dateLisible: '05/09/2026' }))
      .toBe('valeurs lues et intégrées — analyse par IA (vision), déclenchée manuellement, le 05/09/2026');
  });
  it('PastilleStatutPage — grammaire LOT 97 : ton = famille d’état ; lettres NATURE·ORIGINE ; « non identifiée » ne rend RIEN', () => {
    const lu = renderToStaticMarkup(h(PastilleStatutPage, { s: { nature: 'ia', origine: 'manuelle', etat: 'valeurs_lues', derive: false, dateLisible: '05/09/2026' } }));
    expect(lu).toContain('var(--color-svv-blue)');   // bleu PLEIN (valeurs lues)
    expect(lu).toContain('IA·M');                     // NATURE·ORIGINE en lettres (jamais la couleur seule)
    expect(lu).toContain('role="img"');
    const idIa = renderToStaticMarkup(h(PastilleStatutPage, { s: { nature: 'ia', origine: 'manuelle', etat: 'identifiee_non_lue', derive: true, dateLisible: null } }));
    expect(idIa).toContain('var(--color-svv-blue-soft)'); // bleu PÂLE (identifiée non lue)
    const idTexte = renderToStaticMarkup(h(PastilleStatutPage, { s: { nature: 'sans_ia', origine: 'indeterminee', etat: 'identifiee_non_lue', derive: true, dateLisible: null } }));
    expect(idTexte).toContain('texte·?'); // NATURE sans IA + origine indéterminée → distinct de l'IA (jamais écrasés)
    const ecarte = renderToStaticMarkup(h(PastilleStatutPage, { s: { nature: 'ia', origine: 'manuelle', etat: 'ecarte_rgpd', derive: false, dateLisible: null } }));
    expect(ecarte).toContain('var(--color-svv-field)'); // NEUTRE (ni « lu » ni « identifié »)
    expect(renderToStaticMarkup(h(PastilleStatutPage, { s: { nature: 'aucune', origine: null, etat: 'non_identifiee', derive: false, dateLisible: null } }))).toBe('');
  });
...
describe('LETTRAGE (texte) — repères « en projet » en rouge + ◇ dans les LISTES (même nomenclature que le schéma, 4682b58)', () => {
  ...
  it('schemaADeuxCouleursReperes : vrai SEULEMENT si un existant ET un projet coexistent', () => {
    expect(schemaADeuxCouleursReperes([{ enProjet: false }, { enProjet: true }])).toBe(true);
    expect(schemaADeuxCouleursReperes([{ enProjet: false }, { enProjet: false }])).toBe(false); // que des existants
    expect(schemaADeuxCouleursReperes([{ enProjet: true }, { enProjet: true }])).toBe(false);   // que des projets
    expect(schemaADeuxCouleursReperes([{ enProjet: true }])).toBe(false);
    expect(schemaADeuxCouleursReperes([])).toBe(false);
  });

  it('SchemaParcelleTrace : LÉGENDE du lettrage affichée SEULEMENT aux deux couleurs et si les repères sont visibles', () => {
    const boite: Boite = { largeur: 320, hauteur: 240, marge: 12, cadre: { minX: 0, maxX: 30, minY: 0, maxY: 30 } };
    const parcelle = [[{ x: 0, y: 0 }, { x: 30, y: 0 }, { x: 30, y: 30 }, { x: 0, y: 30 }]];
    const carre = (x: number, y: number, c: number) => [{ x, y }, { x: x + c, y }, { x: x + c, y: y + c }, { x, y: y + c }];
    const mixte = attribuerReperes([{ cleabs: 'X', anneau: carre(1, 1, 10), etat: 'En service' }, { cleabs: 'Y', anneau: carre(16, 16, 10), etat: 'En projet' }]);
    const avecReperes: FiltresSchema = { existant: true, futur: true, reperes: true, emprises: false };
    const html = renderToStaticMarkup(h(SchemaParcelleTrace, { boite, parcelle, emprises: [], polygones: mixte, filtres: avecReperes, ecartes: [], calageLambert: [] }));
    expect(html).toContain('data-legende-lettrage="true"');
    expect(html).toContain('Lettres noires : bâtiments existants sur le cadastre');
    expect(html).toContain('Lettres rouges : bâtiments en projet de construction sur le cadastre');
    // une seule couleur (que des existants) → PAS de légende
    const existants = attribuerReperes([{ cleabs: 'X', anneau: carre(1, 1, 10), etat: 'En service' }, { cleabs: 'Y', anneau: carre(16, 16, 10), etat: 'En service' }]);
    expect(renderToStaticMarkup(h(SchemaParcelleTrace, { boite, parcelle, emprises: [], polygones: existants, filtres: avecReperes, ecartes: [], calageLambert: [] }))).not.toContain('data-legende-lettrage');
    // deux couleurs MAIS repères masqués → PAS de légende (suit la case « Afficher les repères… »)
    const sansReperes: FiltresSchema = { existant: true, futur: true, reperes: false, emprises: false };
    expect(renderToStaticMarkup(h(SchemaParcelleTrace, { boite, parcelle, emprises: [], polygones: mixte, filtres: sansReperes, ecartes: [], calageLambert: [] }))).not.toContain('data-legende-lettrage');
  });
