...
export async function POST(request: Request): Promise<Response> {
  const garde = await exigerAdministrateur(request);
  if ('refus' in garde) return garde.refus;
  const body = (await request.json().catch(() => ({}))) as { dossierId?: unknown; familles?: unknown };
  ...
  try {
    const r = await executerDemandePieces(depsReellesDemandePieces(), { dossierId, familles, auteur: 'admin:decision' });
