...
export async function POST(request: Request): Promise<Response> {
  const garde = await exigerAdministrateur(request);
  if ('refus' in garde) return garde.refus;
  const body = (await request.json().catch(() => ({}))) as { action?: unknown; dossierId?: unknown; familles?: unknown; objet?: unknown; corps?: unknown; dateRelance?: unknown; journalId?: unknown; compteCommeRelance?: unknown; destinataires?: unknown; destinatairesAjoutes?: unknown };
  const compteCommeRelance = body.compteCommeRelance === true; // LOT 30 (②) — défaut « ne compte pas » (statu quo)
  // LOT 32 — LISTE de destinataires CHOISIS (facultative) + sous-liste « saisis à la main » (→ enregistrement au carnet commune).
  //   Validés/normalisés dans l'orchestrateur (dédup casse, tous servables, au moins un). Absente → comportement historique.
  const chaines = (v: unknown): string[] | undefined => (Array.isArray(v) ? v.filter((x): x is string => typeof x === 'string') : undefined);
  const destinataires = chaines(body.destinataires);
  const destinatairesAjoutes = chaines(body.destinatairesAjoutes) ?? [];
