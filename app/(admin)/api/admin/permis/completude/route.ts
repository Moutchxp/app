 ...
 *    AUTOMATIQUE NON BLOQUANTE quand la GED a changé (PERF-2). Renvoie le diagnostic à jour. « Lancer le diagnostic complet des documents » (POST /extraire,
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    return Response.json({ erreur: 'recalcul impossible' }, { status: 503 }); // le client dira l'échec et laissera « Lancer le diagnostic complet des documents »
  }
}
