...
async function main(): Promise<void> {
  ...
  for (const c of decision.corps) {
    console.log(`\n  BAT ${c.repere} (${c.nbPieces} pièce(s)) :`);
    ...
    console.log(`    nb_sous_sol = ${c.nbSousSol ? `${c.nbSousSol.valeur} (${c.nbSousSol.confiance})` : '—'}`);
  ...
  console.log(`\n  gardeCorpsAttribué (ex-sommet permis) : ${decision.gardeCorpsAttribue ? `${decision.gardeCorpsAttribue.cote} → ${decision.gardeCorpsAttribue.repere}` : '—'}`);

  const r = await ecrireNiveaux(resolu.dossier.dossierId, decision, MAJ_PAR);
  console.log('\nÉcrit :');
  for (const c of r.corps) {
    console.log(`  « ${c.repere} » #${c.corpsId}${c.cree ? ' (créé)' : ''} — champs : ${c.ecrits.length ? c.ecrits.join(', ') : 'aucun (saisie prioritaire ?)'}`);
